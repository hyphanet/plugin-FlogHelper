...
for x in `cat $1 | sed -s "s/=.*$//" | sed -s "s/^End$//"`
do
	# Let's see if all the entries in the l10n file ($1) are contained in the sources files
	# If an entry isn't contained, it's stored in another temporary file.
	CONTAINS=`cat .temp | sed -s "s/=.*//" | grep "^$x$"`
...
# Show the temp file where all useless strings are stored.
cat .temp2 | sort | uniq 
